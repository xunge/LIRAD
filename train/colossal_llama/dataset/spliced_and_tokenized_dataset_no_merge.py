...
def supervised_tokenize_pretrain(
    ...
    source_text = tokenizer.bos_token + source_text + tokenizer.eos_token
    target_text = tokenizer.bos_token + target_text + tokenizer.eos_token

    tokenized = tokenizer([source_text, target_text])["input_ids"]
    # sequence_labels = deepcopy(sequence_input_ids)

    source_length = len(tokenized[0])
    target_length = len(tokenized[1])
...
def supervised_tokenize_sft(
    ...
    assert (
            tokenizer.bos_token == conversation_template.seps[0] and tokenizer.eos_token == conversation_template.seps[
        1]
