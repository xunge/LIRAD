...
def decompile_pass_rate(testsets, gen_results_repeat, opts, args):
    ...
    for opt, data in avg_stats.items():
        ...
        results[opt] = {"compile_rate": round(compile_rate, 4), "run_rate": round(run_rate, 4)}
        print(
            f"Optimization {opt}: Compile Rate: {compile_rate:.4f}, Run Rate: {run_rate:.4f}"
        )
    num_opts = len(opts)
    mean_compile_rate = round(sum(results[opt]["compile_rate"] for opt in opts) * 100 / num_opts, 2)
    mean_run_rate = round(sum(results[opt]["run_rate"] for opt in opts) * 100 / num_opts, 2)
    results["mean"] = {
        "compile_rate": mean_compile_rate,
        "run_rate": mean_run_rate,
    }

    # 存储到 JSON 文件
    if args.output_result_path:
        with open(args.output_result_path, "w") as f:
